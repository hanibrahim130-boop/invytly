...
export default function GuestRSVPPage() {
  ...
  const [guest, setGuest] = React.useState<FirestoreGuest | null>(null);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const [evt, gst] = await Promise.all([
          getEventFromFirestore(orderId),
          getGuestFromFirestore(orderId, guestId),
        ]);
        if (!active) return;
        setEvent(evt);
        setGuest(gst);
        ...
        if (gst) {
          if (gst.status === "not_opened") {
            await markGuestOpened(orderId, guestId);
          }
          if (gst.status === "attending" || gst.status === "declined") {
            setSubmitted(true);
            setStatus(gst.status);
            setPlusOnes(gst.plusOnes ?? 0);
          } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (status !== "attending" && status !== "declined") {
      errs.status = "Please select Attending or Declined";
    }
    setErrors(errs);
    ...
    try {
      await updateGuestRSVPInFirestore(orderId, guestId, {
        status,
  ...
  if (loaded && !guest) {
    return (
      <Container className="py-32 text-center">
        ...
        <p className="mt-4 text-[color:var(--muted-foreground)]">
          We couldn&apos;t find your invitation. Please contact the host.
        </p>
        <Button href="/" variant="outline" className="mt-8">
          Go home
        </Button>
      ...
      <Container className="py-20">
        <div className="mx-auto max-w-lg text-center">
          ...
          <p className="mt-4 text-lg leading-relaxed text-[color:var(--muted-foreground)]">
            {status === "attending"
              ? `Your response has been recorded. We can't wait to celebrate with${guest?.name ? `, ${guest.name.split(" ")[0]}` : " you"}.`
          ...
          <div className="lg:col-span-7">
            ...
            {guest?.name && (
              <p className="mt-4 text-sm text-[color:var(--muted-foreground)]">
                Hi <strong className="text-[color:var(--foreground)]">{guest.name}</strong>, please RSVP below.
              </p>
            )}
